...
/**
 * 
 * NOT USED ANYWHERE YET. migt be used to abstract out phi functionalities from dphi, argphi, and retphi instructions
 * Compensates controlphis.
 * Inserted with putphiinstructions, if ptsTo set has more than one ASI(ALlocation Site Identifier).
 * @author yash
 *
 */
public abstract class abstractDPhi extends SSAInstruction{

	public OSSAObject defObj;
	public HashMap<InstanceKey, OSSAObject> argObjs;
	public String phiType;
	...
	public abstractDPhi(OSSAObject defObj, HashMap<InstanceKey, OSSAObject>argsObjs) {
		this.defObj = defObj;
...
//			strBuff.append( argObjs.get(asi).toString(asi)+", ");
		...
		for(Iterator<OSSAObject>argobs= argObjs.values().iterator();argobs.hasNext();) {
			strBuff.append( argobs.next()+", ");
		}
